...
If you want to check only for specific version updates, use the `-t` (`--target`) option:

```
-t, --target <value>  Specify the target version to upgrade to
```
...
```sh
npm-upgrade                  # default behavior (latest)
npm-upgrade --target=patch   # only patches
npm-upgrade -t minor         # only minor versions
```
